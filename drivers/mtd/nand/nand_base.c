...
static struct nand_ecclayout nand_oob_224 = {
	.eccbytes = 224,
	.eccpos = {176, 177, 178, 179, 180, 181,
				...
				uint8_t *buf, int page)
{
	...
	for (i = 0 ; eccsteps; eccsteps--, i += eccbytes, p += eccsize) {
		int stat;
		printf("i= %d\n",i);
		printf("code[0-3] =%x %x %x %x ",ecc_code[i],ecc_code[i+1],ecc_code[i+2],ecc_code[i+3]);
		printf("calc[0-3] =%x %x %x %x\n",ecc_calc[i],ecc_calc[i+1],ecc_calc[i+2],ecc_calc[i+3]);
		stat = chip->ecc.correct(mtd, p, &ecc_code[i], &ecc_calc[i]);
		printf("stat= %d\n",stat);
		if (stat < 0)
			mtd->ecc_stats.failed++;
				  ...
				  const uint8_t *buf)
{
	...
	for (i = 0; i < chip->ecc.total; i++){
		chip->oob_poi[eccpos[i]] = ecc_calc[i];
//		printf("i= %d, pos[]= %d, ecc= %x\n",i,eccpos[i],ecc_calc[i]);
	}
...
int nand_scan_tail(struct mtd_info *mtd)
{
	...
	case NAND_ECC_HW:
		...
		if (!chip->ecc.read_page){
			...
			printf("read page hwecc\n");
		}
		if (!chip->ecc.write_page){
			chip->ecc.write_page = nand_write_page_hwecc;
			printf("write page hwecc\n");
		}
