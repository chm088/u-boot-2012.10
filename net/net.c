...
ushort UdpCheckSum(uint saddr, uint daddr, ushort *buffer, uint size)
{
	uchar Buf[1024];
	...
	NetCopyIP((void *)&ph->src, &saddr);
	NetCopyIP((void *)&ph->dst, &daddr);
	ph->mbz = 0;
			...
			int len)
{
	...
	ip->udp_xsum = 0;
	ip->udp_xsum = (UdpCheckSum(NetOurIP, NetServerIP,
			pkt + IP_HDR_SIZE, UDP_HDR_SIZE + len));
}
